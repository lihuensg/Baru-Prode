...
export function RankingPublicPage() {
  const { user } = useAuth();
  const entries = rankingService.getAll();
  const top3 = entries.slice(0, 3);

  return (
    <PublicLayout>
      {/* Hero */}
      <div className="bg-gradient-to-r from-blue-900 to-blue-700 py-14 px-4 text-center">
        <div className="inline-flex items-center justify-center w-14 h-14 bg-amber-400 rounded-2xl shadow-xl mb-5">
          <Trophy className="w-7 h-7 text-white" />
        </div>
        <h1 className="text-3xl sm:text-4xl font-black text-white font-display mb-2">Ranking General</h1>
        <p className="text-blue-200 text-base">Club Deportivo Barú · Prode Mundial 2026</p>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        ...
        {top3.length >= 3 && (
          <div className="grid grid-cols-3 gap-4 mb-10">
            {/* 2nd */}
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 text-center mt-6 card-hover">
            ...
            {/* 1st */}
            <div className="bg-gradient-to-b from-amber-50 to-white rounded-2xl border border-amber-200 shadow-md p-5 text-center card-hover relative">
            ...
            {/* 3rd */}
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 text-center mt-10 card-hover">
